...
// Keyboard driver for graphical mode
#ifdef NO_GRAPHICS
	#define GFX_KEYBOARD_DRIVER KEYBOARD_DRIVER
#else
...
int t8086_init(int boot_from_hdd, unsigned short *disk_sizes, void *p_gd8086) {
	...
	CAST(unsigned)regs16[REG_AX] = disk_sizes[0];
	...
	switch (xlat_opcode_id)
	{
		...
		OPCODE 48: // Emulator-specific 0F xx opcodes
			...
			switch ((char)i_data0)
			{
				...
				OPCODE 2: // DISK_READ
					regs8[REG_AL] = gd8086_readdisk(gd8086, (int)regs8[REG_DL], CAST(unsigned)regs16[REG_BP] << 9, mem + SEGREG(REG_ES, REG_BX,), regs16[REG_AX]);
				OPCODE 3: //DISK_WRITE
					regs8[REG_AL] = gd8086_writedisk(gd8086, (int)regs8[REG_DL], CAST(unsigned)regs16[REG_BP] << 9, mem + SEGREG(REG_ES, REG_BX,), regs16[REG_AX]);
#else
				OPCODE_CHAIN 0: // PUTCHAR_AL
					write(1, regs8, 1)
				OPCODE 1: // GET_RTC
...
int main(int argc, char **argv) {
...
#endif
	int boot_from_hdd = ((argc > 3) && (*argv[3] == '@')) ? argv[3]++, 1 : 0;
	...
	int res = t8086_init(boot_from_hdd, NULL, NULL);
	if (res != 0) {
		return res;
	}
